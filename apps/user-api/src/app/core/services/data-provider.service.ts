 ...
 */
@Injectable()
export abstract class DataService<E extends ApiBaseEntity<E>, R extends MongoRepository<E>> {
  ...
  async update(id: IdType, dto: Partial<E>): Promise<Partial<E>> {
    ...
    try {
      ...
      this.logger.error(`${this.constructor.name}.'/api/nfts/{nftId}'.replace('{nftId}', encodeURI(nftId)) ${error}`);
    }
  }
