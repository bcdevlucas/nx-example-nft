...
import { StringIdGuard } from '@nft/utility';
...
import { UpdateUserConnectionDto } from '@nft/api-modules/user-connection/dto/update-user-connection.dto';
...
import { UserConnectionDto } from '@nft/api-modules/user-connection/dto/user-connection.dto';
import renderInvitationEmailTemplate from '@nft/api-modules/user-connection/invitation-email.template';
...
export class UserController {
  constructor(
    ...
    private nftOwnerService: NftOwnerItemService,
  ...
  async removeUserConnections(@Body() userConnectionDtos: UserConnectionDto[], @Req() req: Request, @Res() res: Response) {
    // eslint-disable-next-line no-useless-catch
    try {
      const nftOwnersResult = await this.nftOwnerService.removeUserConnectionNftOwnerItems(userConnectionDtos);
      if (nftOwnersResult) {
